...
import { Activity, Clock, Star, ChevronRight, Heart, Calendar, TrendingUp } from 'lucide-react';
    ...
    <div className="bg-gray-50 min-h-[calc(100vh-64px)]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-[#1a2b49]">Welcome back, {user.name.split(' ')[0]}</h1>
          ...
          <div className="flex gap-3">
            <div className="bg-white border border-gray-100 rounded-xl px-5 py-3 text-center">
              ...
              <p className="text-xl font-bold text-[#1a2b49]">{bookings.length}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <div className="lg:col-span-8 space-y-6">
                ...
                return (
                  <div key={b.id} className="bg-white border border-gray-100 rounded-xl p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-3">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="font-semibold text-[#1a2b49]">{sp?.name}</h4>
                        <span className="text-xs text-[#1a73e8] font-medium">{sp?.category}</span>
                      </div>
                    </div>
                    <Link to="/booking" className="px-5 py-2 bg-[#1a2b49] text-white rounded-lg text-xs font-semibold hover:bg-[#0f1d35] transition-colors">
                      View Details
                    </Link>
                  </div>
                );
            ...
            <section>
              <div className="flex items-center justify-between mb-4">
                ...
                <Link to="/booking-history" className="text-xs text-[#1a73e8] font-semibold hover:underline">View All</Link>
              </div>
                ...
                {pastBookings.slice(0, 5).map(b => {
                  ...
                  return (
                    <div key={b.id} className="bg-white border border-gray-100 rounded-xl p-4 flex justify-between items-center">
                      <div>
                        <h4 className="font-semibold text-[#1a2b49] text-sm">{sp?.name}</h4>
                      ...
                      <span className="font-bold text-[#1a2b49]">₹{b.totalValue}</span>
                    </div>
                  );
                })}
              </div>
          ...
          <aside className="lg:col-span-4 space-y-6">
            {favorites.length > 0 && (
              <section className="bg-white border border-gray-100 rounded-xl p-5">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="text-sm font-semibold text-[#1a2b49]">Favorites ({favorites.length})</h3>
                </div>
                <div className="space-y-3">
                  {favorites.map(sp => (
                    <Link key={sp.id} to={`/profile/${sp.id}`} className="flex items-center gap-3 group">
                      <img src={sp.avatar} className="w-10 h-10 rounded-lg object-cover flex-shrink-0" />
                      <div className="flex-1 min-w-0">
                        <h4 className="text-sm font-semibold text-[#1a2b49] truncate">{sp.name}</h4>
                        <div className="flex items-center gap-1">
                          <Star className="w-3 h-3 fill-yellow-400 text-yellow-400" />
                          <span className="text-xs text-gray-400">{sp.rating}</span>
                        </div>
                      </div>
                      <ChevronRight className="w-4 h-4 text-gray-300 group-hover:text-[#1a73e8] flex-shrink-0" />
                    </Link>
                  ))}
                </div>
              </section>
            ...
            <section className="bg-white border border-gray-100 rounded-xl p-5">
              <h3 className="text-sm font-semibold text-[#1a2b49] mb-4">Recommended For You</h3>
              <div className="space-y-3">
              ...
              <Link to="/listing" className="block text-center text-xs text-[#1a73e8] font-semibold mt-4 pt-4 border-t border-gray-100 hover:underline">
